...
for k = 1:length(IDs_unique)%for septic data
    ...
    X_s_wave_add = [X_s_wave_add , Fs(6,IDs==IDs_unique(k)).^1'];
...
for k = 1:length(IDn_unique)%for non-septic data
    ...
    X_n_wave_add = [X_n_wave_add , Fn(6,IDn==IDn_unique(k)).^1'];
...
clinical_data_t = load('clinical_data_training.mat');
waveform_data_t = load('waveform_data_training.mat');
...
for k = 1:length(IDs_unique_t)
    ...
    X_s_wave_add_t = (Fs(2,IDs==IDs_unique(k)))'.^3;
    X_s_wave_add_t = [X_s_wave_add_t , Fs(3,IDs==IDs_unique(k)).^.15'];
    X_s_wave_add_t = [X_s_wave_add_t , Fs(5,IDs==IDs_unique(k)).^2'];
    X_s_wave_add_t = [X_s_wave_add_t , Fs(6,IDs==IDs_unique(k)).^1'];
...
for k = 1:length(IDn_unique_t)
    ...
    X_n_wave_add_t = (Fn(2,IDn==IDn_unique(k)))'.^3;
    X_n_wave_add_t = [X_n_wave_add_t , Fn(3,IDn==IDn_unique(k)).^.15'];
    X_n_wave_add_t = [X_n_wave_add_t , Fn(5,IDn==IDn_unique(k)).^2'];
    X_n_wave_add_t = [X_n_wave_add_t , Fn(6,IDn==IDn_unique(k)).^1'];
...
X_test = [X_s_t; X_n_t];
X_test = [X_test, X_test(:,1).*X_test(:,3), X_test(:,1).*X_test(:,5)];
Phat_test = 1./(1+exp(-[ones(size(X_test,1),1) X_test]*B));

septic_wave = (Phat_test > thresh);
P_Y = [ones(length(IDs_unique_t),1); zeros(length(IDn_unique_t),1)];
Phat_P = [];
